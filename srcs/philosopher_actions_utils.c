...
/*   Updated: 2024/09/28 11:23:43 by mmauchre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sleeping_usleep(t_philosopher *philosopher)
{
	...
	while (philosopher->action_time_elapsed < philosopher->shared_data
		->args.time_to_sleep)
	{
...
void	update_meals_number(t_philosopher *philosopher)
{
	...
	if (philosopher->meals_number == 0)
	{
		...
		philosopher->meals_limit_reached = true;
		pthread_mutex_unlock(&philosopher->shared_data->meals_limit[i]);
		pthread_mutex_lock(&philosopher->shared_data->meals_completed);
		philosopher->shared_data->philosophers_meals_completed++;
		pthread_mutex_unlock(&philosopher->shared_data->meals_completed);
	}
}
