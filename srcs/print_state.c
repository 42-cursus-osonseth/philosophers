...
/*   Updated: 2024/09/20 16:39:31 by mmauchre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_thinking(t_philosopher *philosopher)
{
	...
	printf(COLOR_GREEN "%13ld" COLOR_RESET COLOR_MAGENTA " Philo\
		%3zu" COLOR_RESET COLOR_YELLOW "    is thinking" COLOR_RESET "\n",
...
void	print_sleeping(t_philosopher *philosopher)
{
	...
	printf(COLOR_GREEN "%13ld" COLOR_RESET COLOR_MAGENTA " Philo\
		%3zu" COLOR_RESET COLOR_PINK "    is sleeping" COLOR_RESET "\n",
		philosopher->shared_data->timestamp, philosopher->id);
	pthread_mutex_unlock(&philosopher->shared_data->print_mutex);
}
...
void	print_taking_forks(t_philosopher *philosopher)
{
	...
	philosopher->shared_data->timestamp = get_timestamp_in_ms()
		- philosopher->shared_data->start_time;
	printf(COLOR_GREEN "%13ld" COLOR_RESET COLOR_MAGENTA " Philo\
		%3zu" COLOR_RESET COLOR_CYAN "    has taken a fork" COLOR_RESET "\n",
		philosopher->shared_data->timestamp, philosopher->id);
	pthread_mutex_unlock(&philosopher->shared_data->print_mutex);
}
...
void	print_eating(t_philosopher *philosopher)
{
	...
	philosopher->action_start_time = get_timestamp_in_ms();
	printf(COLOR_GREEN "%13ld" COLOR_RESET COLOR_MAGENTA " Philo\
		%3zu" COLOR_RESET COLOR_BLUE "     is eating" COLOR_RESET "\n",
		philosopher->shared_data->timestamp, philosopher->id);
	pthread_mutex_unlock(&philosopher->shared_data->print_mutex);
}
